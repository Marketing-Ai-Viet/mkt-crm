...
import { prefillMktPaymentHistories as prefillMktData } from 'src/mkt-core/dev-seeder/prefill-data/prefill-mkt-payment-histories';
import { mktPaymentHistoriesAllView as mktAllView } from 'src/mkt-core/dev-seeder/prefill-view/mkt-payment-history-all.view';

interface SeedModuleOptions {
  workspaceId?: string;
}

const TABLE_NAME = 'All Payment Histories';
const nameSingular = 'mktPaymentHistory';

type viewDefinition = ReturnType<typeof mktAllView>;

@Command({
  name: 'workspace:seed:payment-history-module',
  description: 'Seed module views and data for existing workspace',
})
export class SeedMktPaymentHistoryModuleCommand extends CommandRunner {
  private readonly logger = new Logger(SeedMktPaymentHistoryModuleCommand.name);
