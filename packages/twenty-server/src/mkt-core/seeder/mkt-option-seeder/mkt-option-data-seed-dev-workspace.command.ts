...
import { mktOptionsAllView as mktAllView } from 'src/mkt-core/seeder/mkt-option-seeder/mkt-option-all.view';
import { prefillMktOptions as prefillMktData } from 'src/mkt-core/seeder/mkt-option-seeder/prefill-mkt-options';

interface SeedModuleOptions {
  workspaceId?: string;
}

const TABLE_NAME = 'All Options';
const nameSingular = 'mktOption';

type viewDefinition = ReturnType<typeof mktAllView>;

@Command({
  name: 'workspace:seed:option-module',
  description: 'Seed module views and data for existing workspace',
})
export class SeedMktOptionModuleCommand extends CommandRunner {
  private readonly logger = new Logger(SeedMktOptionModuleCommand.name);
