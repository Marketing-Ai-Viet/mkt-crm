...
import { mktValuesAllView } from 'src/mkt-core/seeder/product-seeder/mkt-value-all.view';
import { prefillMktValues } from 'src/mkt-core/seeder/product-seeder/prefill-mkt-values';
...
export class SeedValueModuleCommand extends CommandRunner {
      ...
      try {
        ...
        if (valueViewId) {
          ...
          await mainDataSource
            ...
            .execute();
          this.logger.log(
            `✅ Inserted new value record with viewId: ${valueViewId}`,
          );
        } else {
          this.logger.warn(
            '⚠️ Could not find viewId for All Product Values view to update value records',
          );
        }
        this.logger.log(
          `✅ Value module seeded for workspace: ${workspace.id}`,
        );
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(
          `❌ Failed to seed value module for workspace ${workspace.id}:`,
          error,
        );
      }
    }
  ...
  private async seedValueModuleForWorkspace(
    workspaceId: string,
  ): Promise<void> {
    this.logger.log(
      `🚀 Starting value module seeding for workspace ${workspaceId}`,
    );
    ...
    this.logger.log(
      `🔍 Debug - Looking for value object with nameSingular: 'mktValue'`,
    );
    this.logger.log(
      `🔍 Debug - Value object found: ${valueObjectMetadata ? 'YES' : 'NO'}`,
    );

    if (!valueObjectMetadata) {
      this.logger.log(
        `Value object not found in workspace ${workspaceId}, skipping...`,
      );
    ...
    await mainDataSource.transaction(
      async (entityManager: WorkspaceEntityManager) => {
        ...
        if (existingView) {
          this.logger.log(
            `Value view already exists for workspace ${workspaceId}. Deleting and recreating...`,
          );
        ...
        if (!valueViewDefinition) {
          this.logger.log(
            `Could not create value view definition for workspace ${workspaceId}`,
          );
        ...
        this.logger.log(
          `🔍 Debug - View definition created with ${valueViewDefinition.fields?.length || 0} fields`,
        );
