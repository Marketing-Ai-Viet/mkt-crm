...
import { mktEmailAllView as mktAllView } from 'src/mkt-core/seeder/email-seeder/mkt-email-all.view';
import { prefillMktEmails as prefillMktData } from 'src/mkt-core/seeder/email-seeder/prefill-mkt-emails';

interface SeedModuleOptions {
  workspaceId?: string;
}

const TABLE_NAME = 'All Emails';
const nameSingular = 'mktEmail';

type viewDefinition = ReturnType<typeof mktAllView>;

@Command({
  name: 'workspace:seed:email-module',
  description: 'Seed module views and data for existing workspace',
})
export class SeedMktEmailModuleCommand extends CommandRunner {
  private readonly logger = new Logger(SeedMktEmailModuleCommand.name);
  constructor(
