...
export class SeedSInvoiceMetadataModuleCommand extends CommandRunner {
  ...
  @Option({
    ...
    description: 'workspace id to seed sinvoice metadata module for',
  })
  ...
  async run(passedParam: string[], options: SeedModuleOptions): Promise<void> {
    ...
    for (const workspace of workspaces) {
      try {
        ...
        // Lấy viewId của view 'All SInvoice Metadata' sau khi seed
        ...
        const viewRow = await mainDataSource
          ...
          .where('view.name = :name', { name: 'All SInvoice Metadata' })
        ...
        if (ViewId) {
          ...
          this.logger.warn(
            '⚠️ Could not find viewId for All SInvoice Metadata view to update Favorite records',
          );
        }
        this.logger.log(
          `✅ SInvoice metadata module seeded for workspace: ${workspace.id}`,
        );
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(
          `❌ Failed to seed sinvoice metadata module for workspace ${workspace.id}:`,
          error,
        );
      }
    }
  ...
  private async seedModuleForWorkspace(workspaceId: string): Promise<void> {
    this.logger.log(
      `🚀 Starting sinvoice metadata module seeding for workspace ${workspaceId}`,
    );
    ...
    // Find sinvoice metadata object metadata
    const itemObjectMetadata = objectMetadataItems.find(
      (item) => item.nameSingular === 'mktSInvoiceMetadata',
    );
    ...
    this.logger.log(
      `🔍 Debug - Looking for sinvoice metadata object with nameSingular: 'mktSInvoiceMetadata'`,
    );
    this.logger.log(
      `🔍 Debug - SInvoice metadata object found: ${itemObjectMetadata ? 'YES' : 'NO'}`,
    );

    if (!itemObjectMetadata) {
      this.logger.log(
        `SInvoice metadata object not found in workspace ${workspaceId}, skipping...`,
      );
    ...
    await mainDataSource.transaction(
      async (entityManager: WorkspaceEntityManager) => {
        // Check if sinvoice metadata view already exists by looking for a view with name 'All SInvoice Metadata'
        ...
        if (existingView) {
          this.logger.log(
            `SInvoice metadata view already exists for workspace ${workspaceId}. Deleting and recreating...`,
          );
          ...
          await entityManager
            ...
            .where('name = :name', { name: 'All SInvoice Metadata' })
            .andWhere('key = :key', { key: 'INDEX' })
            .execute();
        }

        // Create sinvoice metadata view
        const sinvoiceMetadataViewDefinition: SInvoiceMetadataViewDefinition =
          mktSInvoiceMetadataAllView(objectMetadataItems);

        // Seed mkt sinvoice metadata
        await prefillMktSInvoiceMetadata(entityManager, schemaName);

        if (!sinvoiceMetadataViewDefinition) {
          this.logger.log(
            `Could not create sinvoice metadata view definition for workspace ${workspaceId}`,
          );
        ...
        this.logger.log(
          `🔍 Debug - View definition created with ${sinvoiceMetadataViewDefinition.fields?.length || 0} fields`,
        );

        const viewDefinitionWithId = {
          ...sinvoiceMetadataViewDefinition,
          id: uuidv4(),
        };
