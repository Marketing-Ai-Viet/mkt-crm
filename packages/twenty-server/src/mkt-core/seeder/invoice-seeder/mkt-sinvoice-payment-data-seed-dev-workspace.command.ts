...
type SInvoicePaymentViewDefinition = ReturnType<
  typeof mktSInvoicePaymentsAllView
>;

@Command({
  name: 'workspace:seed:sinvoice-payment-module',
  description:
    'Seed sinvoice payment module views and data for existing workspace',
})
export class SeedSInvoicePaymentModuleCommand extends CommandRunner {
  private readonly logger = new Logger(SeedSInvoicePaymentModuleCommand.name);
  ...
  @Option({
    ...
    description: 'workspace id to seed sinvoice payment module for',
  })
  ...
  async run(passedParam: string[], options: SeedModuleOptions): Promise<void> {
    ...
    for (const workspace of workspaces) {
      try {
        ...
        // Lấy viewId của view 'All SInvoice Payments' sau khi seed
        ...
        const viewRow = await mainDataSource
          ...
          .where('view.name = :name', { name: 'All SInvoice Payments' })
        ...
        if (ViewId) {
          ...
          this.logger.warn(
            '⚠️ Could not find viewId for All SInvoice Payments view to update Favorite records',
          );
        }
        this.logger.log(
          `✅ SInvoice Payment module seeded for workspace: ${workspace.id}`,
        );
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(
          `❌ Failed to seed sinvoice payment module for workspace ${workspace.id}:`,
          error,
        );
      }
    }
  ...
  private async seedModuleForWorkspace(workspaceId: string): Promise<void> {
    this.logger.log(
      `🚀 Starting sinvoice payment module seeding for workspace ${workspaceId}`,
    );
    ...
    // Find sinvoice payment object metadata
    const itemObjectMetadata = objectMetadataItems.find(
      (item) => item.nameSingular === 'mktSInvoicePayment',
    );
    ...
    this.logger.log(
      `🔍 Debug - Looking for sinvoice payment object with nameSingular: 'mktSInvoicePayment'`,
    );
    this.logger.log(
      `🔍 Debug - SInvoice Payment object found: ${itemObjectMetadata ? 'YES' : 'NO'}`,
    );

    if (!itemObjectMetadata) {
      this.logger.log(
        `SInvoice Payment object not found in workspace ${workspaceId}, skipping...`,
      );
    ...
    await mainDataSource.transaction(
      async (entityManager: WorkspaceEntityManager) => {
        // Check if sinvoice payment view already exists by looking for a view with name 'All SInvoice Payments'
        ...
        if (existingView) {
          this.logger.log(
            `SInvoice Payment view already exists for workspace ${workspaceId}. Deleting and recreating...`,
          );
          ...
          await entityManager
            ...
            .where('name = :name', { name: 'All SInvoice Payments' })
            .andWhere('key = :key', { key: 'INDEX' })
            .execute();
        }

        // Create sinvoice payment view
        const sinvoicePaymentViewDefinition: SInvoicePaymentViewDefinition =
          mktSInvoicePaymentsAllView(objectMetadataItems);

        // Seed mkt sinvoice payments
        await prefillMktSInvoicePayments(entityManager, schemaName);

        if (!sinvoicePaymentViewDefinition) {
          this.logger.log(
            `Could not create sinvoice payment view definition for workspace ${workspaceId}`,
          );
        ...
        this.logger.log(
          `🔍 Debug - View definition created with ${sinvoicePaymentViewDefinition.fields?.length || 0} fields`,
        );

        const viewDefinitionWithId = {
          ...sinvoicePaymentViewDefinition,
          id: uuidv4(),
        };
        ...
        this.logger.log(
          `✅ SInvoice Payment view created for workspace ${workspaceId}`,
        );
      },
    );
  }
