...
import { mktOrderHistoryAllView as mktAllView } from 'src/mkt-core/seeder/order-seeder/mkt-order-history/mkt-order-history-all.view';
import { prefillMktOrderHistories as prefillMktData } from 'src/mkt-core/seeder/order-seeder/mkt-order-history/prefill-mkt-order-histories';

interface SeedModuleOptions {
  workspaceId?: string;
}

const TABLE_NAME = 'All Order History';
const nameSingular = 'mktOrderHistory';

type viewDefinition = ReturnType<typeof mktAllView>;

@Command({
  name: 'workspace:seed:order-history-module',
  description: 'Seed module views and data for existing workspace',
})
export class SeedMktOrderHistoryModuleCommand extends CommandRunner {
  private readonly logger = new Logger(SeedMktOrderHistoryModuleCommand.name);
