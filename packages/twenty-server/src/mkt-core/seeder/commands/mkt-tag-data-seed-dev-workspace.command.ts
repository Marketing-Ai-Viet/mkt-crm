...
type TagViewDefinition = ReturnType<typeof mktTagsAllView>;

@Command({
  name: 'workspace:seed:tag-module',
  description: 'Seed tag module views and data for existing workspace',
})
export class SeedTagModuleCommand extends CommandRunner {
  private readonly logger = new Logger(SeedTagModuleCommand.name);
  ...
  @Option({
    ...
    description: 'workspace id to seed tag module for',
  })
  ...
  async run(passedParam: string[], options: SeedModuleOptions): Promise<void> {
    ...
    for (const workspace of workspaces) {
      try {
        ...
        // get viewId of view 'All Tag' after seed
        ...
        const viewRow = await mainDataSource
          ...
          .where('view.name = :name', { name: 'All Tags' })
        ...
        if (ViewId) {
          ...
          this.logger.warn(
            '⚠️ Could not find viewId for All Tags view to update Favorite records',
          );
        }
        this.logger.log(`✅ Tag module seeded for workspace: ${workspace.id}`);
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(
          `❌ Failed to seed tag module for workspace ${workspace.id}:`,
          error,
        );
      }
    }
  ...
  private async seedModuleForWorkspace(workspaceId: string): Promise<void> {
    this.logger.log(
      `🚀 Starting tag module seeding for workspace ${workspaceId}`,
    );
    ...
    // Find tag object metadata
    const itemObjectMetadata = objectMetadataItems.find(
      (item) => item.nameSingular === 'mktTag',
    );
    ...
    this.logger.log(
      `🔍 Debug - Looking for combo object with nameSingular: 'mktTag'`,
    );
    this.logger.log(
    ...
    if (!itemObjectMetadata) {
      this.logger.log(
        `Tag object not found in workspace ${workspaceId}, skipping...`,
      );
    ...
    await mainDataSource.transaction(
      async (entityManager: WorkspaceEntityManager) => {
        // Check if tag view already exists by looking for a view with name 'All Tags'
        ...
        if (existingView) {
          this.logger.log(
            `Tag view already exists for workspace ${workspaceId}. Deleting and recreating...`,
          );
          ...
          await entityManager
            ...
            .where('name = :name', { name: 'All Tags' })
            .andWhere('key = :key', { key: 'INDEX' })
            .execute();
        }

        // Create tag view
        const tagViewDefinition: TagViewDefinition =
          mktTagsAllView(objectMetadataItems);

        // Seed mkt tags
        await prefillMktTags(entityManager, schemaName);

        if (!tagViewDefinition) {
          this.logger.log(
            `Could not create tag view definition for workspace ${workspaceId}`,
          );
        ...
        this.logger.log(
          `🔍 Debug - View definition created with ${tagViewDefinition.fields?.length || 0} fields`,
        );
        ...
        this.logger.log(`✅ Tag view created for workspace ${workspaceId}`);
      },
    );
  }
}
