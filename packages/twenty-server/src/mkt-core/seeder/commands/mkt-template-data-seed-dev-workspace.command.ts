...
type TemplateViewDefinition = ReturnType<typeof mktTemplatesAllView>;

@Command({
  name: 'workspace:seed:template-module',
  description: 'Seed template module views and data for existing workspace',
})
export class SeedTemplateModuleCommand extends CommandRunner {
  private readonly logger = new Logger(SeedTemplateModuleCommand.name);
  ...
  @Option({
    ...
    description: 'workspace id to seed template module for',
  })
  ...
  async run(passedParam: string[], options: SeedModuleOptions): Promise<void> {
    ...
    for (const workspace of workspaces) {
      try {
        ...
        // Lấy viewId của view 'All Template' sau khi seed
        ...
        const viewRow = await mainDataSource
          ...
          .where('view.name = :name', { name: 'All Templates' })
        ...
        if (ViewId) {
          ...
          this.logger.warn(
            '⚠️ Could not find viewId for All Templates view to update Favorite records',
          );
        }
        this.logger.log(
          `✅ Template module seeded for workspace: ${workspace.id}`,
        );
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(
          `❌ Failed to seed template module for workspace ${workspace.id}:`,
          error,
        );
      }
    }
  ...
  private async seedModuleForWorkspace(workspaceId: string): Promise<void> {
    this.logger.log(
      `🚀 Starting template module seeding for workspace ${workspaceId}`,
    );
    ...
    // Find template object metadata
    const itemObjectMetadata = objectMetadataItems.find(
      (item) => item.nameSingular === 'mktTemplate',
    );
    ...
    this.logger.log(
      `🔍 Debug - Looking for template object with nameSingular: 'mktTemplate'`,
    );
    this.logger.log(
      `🔍 Debug - Template object found: ${itemObjectMetadata ? 'YES' : 'NO'}`,
    );

    if (!itemObjectMetadata) {
      this.logger.log(
        `Template object not found in workspace ${workspaceId}, skipping...`,
      );
    ...
    await mainDataSource.transaction(
      async (entityManager: WorkspaceEntityManager) => {
        // Check if template view already exists by looking for a view with name 'All Templates'
        ...
        if (existingView) {
          this.logger.log(
            `Template view already exists for workspace ${workspaceId}. Deleting and recreating...`,
          );
          ...
          await entityManager
            ...
            .where('name = :name', { name: 'All Templates' })
            .andWhere('key = :key', { key: 'INDEX' })
            .execute();
        }

        // Create template view
        const templateViewDefinition: TemplateViewDefinition =
          mktTemplatesAllView(objectMetadataItems);

        // Seed mkt templates
        await prefillMktTemplates(entityManager, schemaName);

        if (!templateViewDefinition) {
          this.logger.log(
            `Could not create template view definition for workspace ${workspaceId}`,
          );
        ...
        this.logger.log(
          `🔍 Debug - View definition created with ${templateViewDefinition.fields?.length || 0} fields`,
        );
        ...
        this.logger.log(
          `✅ Template view created for workspace ${workspaceId}`,
        );
      },
    );
  }
