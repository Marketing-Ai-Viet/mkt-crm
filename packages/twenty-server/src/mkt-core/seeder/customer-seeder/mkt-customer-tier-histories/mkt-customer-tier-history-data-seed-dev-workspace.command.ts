...
type TierHistoryViewDefinition = ReturnType<
  typeof mktCustomerTierHistoriesAllView
>;

@Command({
  name: 'workspace:seed:customer-tier-history-module',
  description:
    'Seed customer tier history module views and data for existing workspace',
})
export class SeedCustomerTierHistoryModuleCommand extends CommandRunner {
  private readonly logger = new Logger(
    SeedCustomerTierHistoryModuleCommand.name,
  );
  ...
  @Option({
    ...
    description: 'workspace id to seed customer tier history module for',
  })
  ...
  async run(passedParam: string[], options: SeedModuleOptions): Promise<void> {
    ...
    for (const workspace of workspaces) {
      try {
        ...
        // Lấy viewId của view 'All Tier Histories' sau khi seed
        ...
        if (viewId) {
          ...
          this.logger.warn(
            '⚠️ Could not find viewId for All Tier Histories view to update Favorite records',
          );
        }

        this.logger.log(
          `✅ Customer tier history module seeded for workspace: ${workspace.id}`,
        );
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(
          `❌ Failed to seed customer tier history module for workspace ${workspace.id}:`,
          error,
        );
      }
    }
  ...
  private async seedModuleForWorkspace(workspaceId: string): Promise<void> {
    this.logger.log(
      `🚀 Starting customer tier history module seeding for workspace ${workspaceId}`,
    );
    ...
    // Find customer tier history object metadata
    const itemObjectMetadata = objectMetadataItems.find(
      (item) => item.nameSingular === 'mktCustomerTierHistory',
    );

    this.logger.log(
      `🔍 Debug - Looking for tier history object with nameSingular: 'mktCustomerTierHistory'`,
    );
    this.logger.log(
      `🔍 Debug - Tier history object found: ${itemObjectMetadata ? 'YES' : 'NO'}`,
    );

    if (!itemObjectMetadata) {
      this.logger.log(
        `Customer tier history object not found in workspace ${workspaceId}, skipping...`,
      );
    ...
    await mainDataSource.transaction(
      async (entityManager: WorkspaceEntityManager) => {
        ...
        if (existingView) {
          this.logger.log(
            `Tier history view already exists for workspace ${workspaceId}. Deleting and recreating...`,
          );
        ...
        const tierHistoryViewDefinition: TierHistoryViewDefinition =
        ...
        if (!tierHistoryViewDefinition) {
          this.logger.log(
            `Could not create tier history view definition for workspace ${workspaceId}`,
          );
        ...
        this.logger.log(
          `🔍 Debug - View definition created with ${tierHistoryViewDefinition.fields?.length || 0} fields`,
        );
