...
@Command({
  name: 'workspace:seed:data-access-policy-module',
  description:
    'Seed data access policy module views and data for existing workspace',
})
export class SeedMktDataAccessPolicyCommand extends CommandRunner {
  private readonly logger = new Logger(SeedMktDataAccessPolicyCommand.name);
  ...
  @Option({
    ...
    description: 'workspace id to seed data access policy module for',
  })
  ...
  async run(
    passedParam: string[],
    options: SeedDataAccessPolicyModuleOptions,
  ): Promise<void> {
    ...
    for (const workspace of workspaces) {
      try {
        await this.seedDataAccessPolicyModuleForWorkspace(workspace.id);
        this.logger.log(
          `✅ Data access policy module seeded for workspace: ${workspace.id}`,
        );
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(
          `❌ Failed to seed data access policy module for workspace ${workspace.id}:`,
          error,
        );
      }
    }
  ...
  private async seedDataAccessPolicyModuleForWorkspace(
    workspaceId: string,
  ): Promise<void> {
    this.logger.log(
      `🚀 Starting data access policy module seeding for workspace ${workspaceId}`,
    );
    ...
    // Find data access policy object metadata
    const dataAccessPolicyObjectMetadata = objectMetadataItems.find(
      (item) => item.nameSingular === 'mktDataAccessPolicy',
    );

    if (!dataAccessPolicyObjectMetadata) {
      this.logger.log(
        `Data access policy object not found in workspace ${workspaceId}, skipping...`,
      );
    ...
    await mainDataSource.transaction(
      async (entityManager: WorkspaceEntityManager) => {
        // Seed data access policy data first
        await prefillMktDataAccessPolicies(entityManager, schemaName);

        // Create all views
        const viewDefinitions = [
          mktDataAccessPoliciesAllView(objectMetadataItems),
          mktDataAccessPoliciesActiveView(objectMetadataItems),
          mktDataAccessPoliciesDepartmentView(objectMetadataItems),
          mktDataAccessPoliciesMemberView(objectMetadataItems),
        ];
