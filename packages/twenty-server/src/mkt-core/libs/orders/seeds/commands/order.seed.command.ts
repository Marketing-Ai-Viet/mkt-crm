  ...
  async run(
    ...
    for (const workspace of workspaces) {
      try {
        ...
        // Lấy viewId của view 'All Orders' sau khi seed
        ...
        if (orderViewId) {
          ...
          await mainDataSource
            ...
            .execute();
          this.logger.log(`✅ Inserted new Favorite record with viewId: ${orderViewId}`);
        } else {
          this.logger.warn('⚠️ Could not find viewId for All Orders view to update Favorite records');
        }
        this.logger.log(`✅ Order module seeded for workspace: ${workspace.id}`);
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(`❌ Failed to seed order module for workspace ${workspace.id}:`, error);
      }
    }
  }

  private async seedOrderModuleForWorkspace(workspaceId: string): Promise<void> {
    this.logger.log(`🚀 Starting order module seeding for workspace ${workspaceId}`);
    ...
    const orderObjectMetadata = objectMetadataItems.find(
      (item) => item.nameSingular === 'order'
    );

    this.logger.log(`🔍 Debug - All objects in workspace: ${objectMetadataItems.map(item => `${item.nameSingular}(${item.standardId})`).join(', ')}`);
    this.logger.log(`🔍 Debug - Looking for order object with nameSingular: 'order'`);
    this.logger.log(`🔍 Debug - Order object found: ${orderObjectMetadata ? 'YES' : 'NO'}`);

    if (!orderObjectMetadata) {
      this.logger.log(`Order object not found in workspace ${workspaceId}, skipping...`);
      return;
    }
    ...
    await mainDataSource.transaction(async (entityManager: WorkspaceEntityManager) => {
      ...
      if (existingView) {
        this.logger.log(`Order view already exists for workspace ${workspaceId}. Deleting and recreating...`);
      ...
      // Create order view
      const orderViewDefinition = ordersAllView(objectMetadataItems);
      await prefillOrders(entityManager, schemaName);
      
      if (!orderViewDefinition) {
        this.logger.log(`Could not create order view definition for workspace ${workspaceId}`);
        return;
      }

      this.logger.log(`🔍 Debug - View definition created with ${orderViewDefinition.fields?.length || 0} fields`);
