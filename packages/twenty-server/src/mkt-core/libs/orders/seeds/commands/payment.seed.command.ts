...
interface SeedPaymentModuleOptions {
  workspaceId?: string;
}

@Command({
  name: 'workspace:seed:payment-module',
  description: 'Seed payment module views and data for existing workspace',
})
export class SeedPaymentModuleCommand extends CommandRunner {
  private readonly logger = new Logger(SeedPaymentModuleCommand.name);
  ...
  @Option({
    ...
    description: 'workspace id to seed payment module for',
  })
  ...
  async run(
    passedParam: string[],
    options: SeedPaymentModuleOptions,
  ): Promise<void> {
    ...
    for (const workspace of workspaces) {
      try {
        await this.seedPaymentModuleForWorkspace(workspace.id);
        // Lấy viewId của view 'All Payments' sau khi seed
        ...
        const viewRow = await mainDataSource
          ...
          .where('view.name = :name', { name: 'All Payments' })
          .andWhere('view.key = :key', { key: 'INDEX' })
          .getRawOne();
        const paymentViewId = viewRow?.id;
        if (paymentViewId) {
          ...
          await mainDataSource
            ...
            .values([{ viewId: paymentViewId }])
            .execute();
          this.logger.log(`✅ Inserted new Favorite record with viewId: ${paymentViewId}`);
        } else {
          this.logger.warn('⚠️ Could not find viewId for All Payments view to update Favorite records');
        }
        this.logger.log(`✅ Payment module seeded for workspace: ${workspace.id}`);
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(`❌ Failed to seed payment module for workspace ${workspace.id}:`, error);
      }
    }
  }

  private async seedPaymentModuleForWorkspace(workspaceId: string): Promise<void> {
    this.logger.log(`🚀 Starting payment module seeding for workspace ${workspaceId}`);
    ...
    // Find payment object metadata
    const paymentObjectMetadata = objectMetadataItems.find(
      (item) => item.nameSingular === 'mktPayment'
    );

    this.logger.log(`🔍 Debug - All objects in workspace: ${objectMetadataItems.map(item => `${item.nameSingular}(${item.standardId})`).join(', ')}`);
    this.logger.log(`🔍 Debug - Looking for payment object with nameSingular: 'mktPayment'`);
    this.logger.log(`🔍 Debug - Payment object found: ${paymentObjectMetadata ? 'YES' : 'NO'}`);

    if (!paymentObjectMetadata) {
      this.logger.log(`Payment object not found in workspace ${workspaceId}, skipping...`);
      return;
    }
    ...
    await mainDataSource.transaction(async (entityManager: WorkspaceEntityManager) => {
      // Check if payment view already exists by looking for a view with name 'All Payments'
      ...
      if (existingView) {
        this.logger.log(`Payment view already exists for workspace ${workspaceId}. Deleting and recreating...`);
        ...
        await entityManager
          ...
          .where('name = :name', { name: 'All Payments' })
          .andWhere('key = :key', { key: 'INDEX' })
          .execute();
      }

      // Create payment view
      const paymentViewDefinition = paymentsAllView(objectMetadataItems);
      await prefillPayments(entityManager, schemaName);
      
      if (!paymentViewDefinition) {
        this.logger.log(`Could not create payment view definition for workspace ${workspaceId}`);
        return;
      }

      this.logger.log(`🔍 Debug - View definition created with ${paymentViewDefinition.fields?.length || 0} fields`);

      const viewDefinitionWithId = {
        ...paymentViewDefinition,
        id: uuidv4(),
      };
      ...
      this.logger.log(`✅ Payment view created for workspace ${workspaceId}`);
    });
  }
}
