...
import { prefillOrderItems } from 'src/mkt-core/libs/orders/seeds/prefill-order-item.seed';
import { orderItemsAllView } from 'src/mkt-core/libs/orders/view/order-item-all.view';
import { WorkspaceActivationStatus } from 'twenty-shared/workspace';
import { v4 as uuidv4 } from 'uuid';

interface SeedOrderItemItemModuleOptions {
  workspaceId?: string;
}

@Command({
  name: 'workspace:seed:orderItem-item-module',
  description: 'Seed orderItem item module views and data for existing workspace',
})
export class SeedOrderItemItemModuleCommand extends CommandRunner {
  private readonly logger = new Logger(SeedOrderItemItemModuleCommand.name);
  ...
  @Option({
    ...
    description: 'workspace id to seed orderItem item module for',
  })
  ...
  async run(
    passedParam: string[],
    options: SeedOrderItemItemModuleOptions,
  ): Promise<void> {
    ...
    for (const workspace of workspaces) {
      try {
        ...
        // Lấy viewId của view 'All OrderItems' sau khi seed
        ...
        const viewRow = await mainDataSource
          ...
          .where('view.name = :name', { name: 'All OrderItems' })
        ...
        if (orderItemViewId) {
          ...
          await mainDataSource
            ...
            .execute();
          this.logger.log(`✅ Inserted new Favorite record with viewId: ${orderItemViewId}`);
        } else {
          this.logger.warn('⚠️ Could not find viewId for All OrderItems view to update Favorite records');
        }
        this.logger.log(`✅ OrderItem module seeded for workspace: ${workspace.id}`);
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(`❌ Failed to seed orderItem module for workspace ${workspace.id}:`, error);
      }
    }
  }

  private async seedOrderItemModuleForWorkspace(workspaceId: string): Promise<void> {
    this.logger.log(`🚀 Starting orderItem module seeding for workspace ${workspaceId}`);
    ...
    // Find orderItem object metadata
    const orderItemObjectMetadata = objectMetadataItems.find(
      (item) => item.nameSingular === 'orderItem'
    );

    this.logger.log(`🔍 Debug - All objects in workspace: ${objectMetadataItems.map(item => `${item.nameSingular}(${item.standardId})`).join(', ')}`);
    this.logger.log(`🔍 Debug - Looking for orderItem object with nameSingular: 'orderItem'`);
    this.logger.log(`🔍 Debug - OrderItem object found: ${orderItemObjectMetadata ? 'YES' : 'NO'}`);

    if (!orderItemObjectMetadata) {
      this.logger.log(`OrderItem object not found in workspace ${workspaceId}, skipping...`);
      return;
    }
    ...
    await mainDataSource.transaction(async (entityManager: WorkspaceEntityManager) => {
      // Check if orderItem view already exists by looking for a view with name 'All OrderItems'
      ...
      if (existingView) {
        this.logger.log(`OrderItem view already exists for workspace ${workspaceId}. Deleting and recreating...`);
        ...
        await entityManager
          ...
          .where('name = :name', { name: 'All OrderItems' })
          .andWhere('key = :key', { key: 'INDEX' })
          .execute();
      }

      // Create orderItem view
      const orderItemViewDefinition = orderItemsAllView(objectMetadataItems);
      await prefillOrderItems(entityManager, schemaName);
      
      if (!orderItemViewDefinition) {
        this.logger.log(`Could not create orderItem view definition for workspace ${workspaceId}`);
        return;
      }

      this.logger.log(`🔍 Debug - View definition created with ${orderItemViewDefinition.fields?.length || 0} fields`);
      ...
      this.logger.log(`✅ OrderItem view created for workspace ${workspaceId}`);
    });
  }
}
