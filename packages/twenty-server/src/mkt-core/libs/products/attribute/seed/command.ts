...
import { productAttributesAllView } from './view';
import { prefillProductAttributes } from './data'
...
@Command({
  ...
  description: 'Seed product attribute module views and data for existing workspace',
})
export class SeedAttributeModuleCommand extends CommandRunner {
  ...
  @Option({
    ...
    description: 'workspace id to seed product attribute module for',
  })
      ...
      try {
        ...
        if (attributeViewId) {
          ...
          await mainDataSource
            ...
            .execute();
          this.logger.log(`✅ Inserted new Favorite record with viewId: ${attributeViewId}`);
        } else {
          this.logger.warn('⚠️ Could not find viewId for All Product Attributes view to update Favorite records');
        }
        this.logger.log(`✅ Product attribute module seeded for workspace: ${workspace.id}`);
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(`❌ Failed to seed product attribute module for workspace ${workspace.id}:`, error);
      }
    }
  }

  private async seedAttributeModuleForWorkspace(workspaceId: string): Promise<void> {
    this.logger.log(`🚀 Starting product attribute module seeding for workspace ${workspaceId}`);
    ...
    await mainDataSource.transaction(async (entityManager: WorkspaceEntityManager) => {
      ...
      if (existingView) {
        this.logger.log(`Product attribute view already exists for workspace ${workspaceId}. Deleting and recreating...`);
      ...
      const attributeViewDefinition = productAttributesAllView(objectMetadataItems);
      await prefillProductAttributes(entityManager, schemaName);
      ...
      this.logger.log(`✅ Product attribute view created for workspace ${workspaceId}`);
    });
  }
}
