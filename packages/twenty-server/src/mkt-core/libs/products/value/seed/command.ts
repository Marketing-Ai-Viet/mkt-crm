...
import { prefillAttributeValues } from './data';
import { attributeValuesAllView } from './view';

interface SeedAttributeValueModuleOptions {
  workspaceId?: string;
}

@Command({
  name: 'workspace:seed:attribute-value-module',
  description: 'Seed attribute value module views and data for existing workspace',
})
export class SeedAttributeValueModuleCommand extends CommandRunner {
  private readonly logger = new Logger(SeedAttributeValueModuleCommand.name);
  ...
  @Option({
    ...
    description: 'workspace id to seed attribute value module for',
  })
  ...
  async run(
    passedParam: string[],
    options: SeedAttributeValueModuleOptions,
  ): Promise<void> {
    ...
    for (const workspace of workspaces) {
      try {
        await this.seedAttributeValueModuleForWorkspace(workspace.id);
        // Lấy viewId của view 'All Attribute Values' sau khi seed
        ...
        const viewRow = await mainDataSource
          ...
          .where('view.name = :name', { name: 'All Attribute Values' })
        ...
        if (valueViewId) {
          ...
          this.logger.warn('⚠️ Could not find viewId for All Attribute Values view to update Favorite records');
        }
        this.logger.log(`✅ Attribute value module seeded for workspace: ${workspace.id}`);
        await this.workspaceCacheStorageService.flush(workspace.id, undefined);
      } catch (error) {
        this.logger.error(`❌ Failed to seed attribute value module for workspace ${workspace.id}:`, error);
      }
    }
  }

  private async seedAttributeValueModuleForWorkspace(workspaceId: string): Promise<void> {
    this.logger.log(`🚀 Starting attribute value module seeding for workspace ${workspaceId}`);
    ...
    const valueObjectMetadata = objectMetadataItems.find(
      (item) => item.nameSingular === 'mktAttributeValue'
    );

    this.logger.log(`🔍 Debug - All objects in workspace: ${objectMetadataItems.map(item => `${item.nameSingular}(${item.standardId})`).join(', ')}`);
    this.logger.log(`🔍 Debug - Looking for value object with nameSingular: 'mktAttributeValue'`);
    this.logger.log(`🔍 Debug - Value object found: ${valueObjectMetadata ? 'YES' : 'NO'}`);

    if (!valueObjectMetadata) {
      this.logger.log(`Attribute value object not found in workspace ${workspaceId}, skipping...`);
      return;
    }
    ...
    await mainDataSource.transaction(async (entityManager: WorkspaceEntityManager) => {
      // Check if value view already exists by looking for a view with name 'All Attribute Values'
      ...
      if (existingView) {
        this.logger.log(`Attribute value view already exists for workspace ${workspaceId}. Deleting and recreating...`);
        ...
        await entityManager
          ...
          .where('name = :name', { name: 'All Attribute Values' })
          .andWhere('key = :key', { key: 'INDEX' })
          .execute();
      }

      // Create value view
      const valueViewDefinition = attributeValuesAllView(objectMetadataItems);
      await prefillAttributeValues(entityManager, schemaName);
      ...
      this.logger.log(`✅ Attribute value view created for workspace ${workspaceId}`);
    });
  }
}
