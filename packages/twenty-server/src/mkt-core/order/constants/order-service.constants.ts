...
export const STATUS_ACTION_MAP: Record<ORDER_STATUS, ORDER_ACTION> = {
  ...
  [ORDER_STATUS.CONFIRMED]: ORDER_ACTION.CONFIRM_ORDER,
  [ORDER_STATUS.LICENSE_PENDING]: ORDER_ACTION.CONFIRM_ORDER,
  [ORDER_STATUS.PROCESSING]: ORDER_ACTION.CONFIRM_ORDER,
  [ORDER_STATUS.COMPLETED]: ORDER_ACTION.COMPLETE,
  [ORDER_STATUS.LICENSE_FAILED]: ORDER_ACTION.CANCEL, // Cần xử lý thủ công
  [ORDER_STATUS.LOCKED]: ORDER_ACTION.LOCK_OVERDUE,
...
export const VALID_STATUS_TRANSITIONS: Record<ORDER_STATUS, ORDER_STATUS[]> = {
  [ORDER_STATUS.DRAFT]: [
    ...
    ORDER_STATUS.CONFIRMED, // New flow: DRAFT → CONFIRMED (skip PENDING_PAYMENT)
    ORDER_STATUS.TRIAL,
  ...
  [ORDER_STATUS.CONFIRMED]: [
    ORDER_STATUS.LICENSE_PENDING, // Async flow: CONFIRMED → LICENSE_PENDING (jobs enqueued)
    ORDER_STATUS.PROCESSING, // Sync flow: CONFIRMED → PROCESSING (license created)
    ORDER_STATUS.COMPLETED,
    ORDER_STATUS.REFUND,
    ORDER_STATUS.REFUND_PARTIAL,
    ORDER_STATUS.BLOCKED,
  ],
  // Async flow: LICENSE_PENDING - Jobs đã enqueue, đang chờ worker xử lý
  [ORDER_STATUS.LICENSE_PENDING]: [
    ORDER_STATUS.PROCESSING, // All licenses created successfully
    ORDER_STATUS.LICENSE_FAILED, // Some license jobs failed
    ORDER_STATUS.CANCELED,
  ],
  // New payment flow: PROCESSING - License đã tạo, đang chờ thanh toán
  [ORDER_STATUS.PROCESSING]: [
    ORDER_STATUS.COMPLETED, // Payment confirmed
    ORDER_STATUS.LOCKED, // Payment overdue
    ORDER_STATUS.CANCELED,
  ],
  [ORDER_STATUS.COMPLETED]: [ORDER_STATUS.REFUND, ORDER_STATUS.REFUND_PARTIAL],
  // Async flow: LICENSE_FAILED - Tạo license thất bại, cần xử lý thủ công
  [ORDER_STATUS.LICENSE_FAILED]: [
    ORDER_STATUS.LICENSE_PENDING, // Retry tạo license
    ORDER_STATUS.CANCELED,
  ],
  // New payment flow: LOCKED - Bị khóa do quá hạn thanh toán
  [ORDER_STATUS.LOCKED]: [
    ORDER_STATUS.COMPLETED, // Late payment - unlock and complete
    ORDER_STATUS.CANCELED,
  ],
  [ORDER_STATUS.OVERDUE]: [
    ORDER_STATUS.PENDING_PAYMENT,
...
> = {
  [ORDER_ACTION.CONFIRM_ORDER]: MKT_ORDER_EVENT_TYPES.ORDER_CONFIRMED,
  [ORDER_ACTION.PAYMENT_CONFIRMED]: MKT_ORDER_EVENT_TYPES.PAYMENT_CONFIRMED,
...
export const VALID_ACTIONS_BY_STATUS: Record<ORDER_STATUS, ORDER_ACTION[]> = {
  ...
  [ORDER_STATUS.CONFIRMED]: [
    ORDER_ACTION.CONFIRM_ORDER, // Confirm → create license → PROCESSING or LICENSE_PENDING
    ORDER_ACTION.COMPLETE,
    ORDER_ACTION.REFUND,
    ORDER_ACTION.REFUND_PARTIAL,
    ORDER_ACTION.BLOCK,
  ],
  // LICENSE_PENDING - Jobs đã enqueue, đang chờ worker xử lý (async flow)
  [ORDER_STATUS.LICENSE_PENDING]: [
    ORDER_ACTION.CANCEL, // Hủy đơn (jobs sẽ bị orphaned)
  ],
  // PROCESSING - License đã tạo, đang chờ thanh toán
  [ORDER_STATUS.PROCESSING]: [
    ORDER_ACTION.PAYMENT_CONFIRMED, // Payment OK → COMPLETED
    ORDER_ACTION.LOCK_OVERDUE, // Payment overdue → LOCKED
    ORDER_ACTION.CANCEL,
  ],
  [ORDER_STATUS.OVERDUE]: [ORDER_ACTION.CONFIRM_ORDER, ORDER_ACTION.CANCEL],
  [ORDER_STATUS.COMPLETED]: [ORDER_ACTION.REFUND, ORDER_ACTION.REFUND_PARTIAL],
  // LICENSE_FAILED - Tạo license thất bại, cần xử lý thủ công
  [ORDER_STATUS.LICENSE_FAILED]: [
    ORDER_ACTION.CANCEL, // Hủy đơn
  ],
  // LOCKED - Bị khóa do quá hạn thanh toán
  [ORDER_STATUS.LOCKED]: [
    ORDER_ACTION.UNLOCK_AFTER_PAYMENT, // Late payment → COMPLETED
    ORDER_ACTION.CANCEL,
  ],
  [ORDER_STATUS.CANCELED]: [],
  [ORDER_STATUS.REFUND]: [],
 ...
 * - CONFIRM_ORDER: Create license with PENDING_PAYMENT status
